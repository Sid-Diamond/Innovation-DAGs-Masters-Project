...
from scipy.special import gamma as gamma_func, betaln as betaln_func, loggamma
...
def pmf_from_beta(self, k, beta) -> np.ndarray:
    ...
    if np.any(pos_mask):
        k_pos = k[pos_mask]
        # Use loggamma to avoid overflow
        log_ratio = (loggamma(k_pos + alpha + gamma) - loggamma(alpha + gamma) 
                     - loggamma(k_pos + alpha) + loggamma(alpha))
        result[pos_mask] = p0 * np.exp(np.clip(log_ratio, -700, 700))
...
def _mc_single_sim_worker(args):
    """
    Top-level worker function for ProcessPoolExecutor.
    args is a tuple: (base_net_params, node_type, a, b, beta_theory_arr, seed)
    """
    (base_net_params, node_type, a, b, beta_theory_arr, seed) = args

    gof = GoFDiagnostics()
    return gof._mc_single_sim(base_net_params=base_net_params,node_type=node_type,
        a=a,b=b,beta_theory_arr=beta_theory_arr,seed=seed,)
...
class DirectedHomophilicNetwork:
    ...
    def homophilic_preferential_attachment(self, n_nodes_so_far: int) -> np.ndarray:
        """Select m_edges targets using cached in-degrees (spliced)."""
        in_deg = self.in_degrees[:n_nodes_so_far]
        types = self.node_types[:n_nodes_so_far]

        lambda_vals = np.empty_like(in_deg, dtype=float)
        mu_vals = np.empty_like(in_deg, dtype=float)

        a_mask = (types == 0)
        b_mask = ~a_mask

        lambda_vals[a_mask] = self.lambda_a
        lambda_vals[b_mask] = self.lambda_b
        mu_vals[a_mask] = self.mu['a']
        mu_vals[b_mask] = self.mu['b']
    ...
    # Initial random edges (vectorised target pool & counts)
        all_nodes = np.arange(self.n0, dtype=int)

        for source in range(self.n0):
            mask = all_nodes != source
            ...
            t_types = self.node_types[targets]
            self.in_edges_a_count += np.count_nonzero(t_types == 0)
            self.in_edges_b_count += np.count_nonzero(t_types == 1)

        # Add nodes with preferential attachment (vectorised counts)
    ...
    def pmf_from_beta(self, k, beta) -> np.ndarray:
        ...
        if np.any(pos_mask):
            k_pos = k[pos_mask]
            # Use betaln for numerical stability (same as original)
            log_ratio = betaln_func(k_pos, alpha + gamma) - betaln_func(k_pos, alpha)
    ...
    def cdf_from_beta_truncated(self, k, beta, kmin: int, kmax: int) -> np.ndarray:
        ...
        # If not cached, build and store (support, CDF) for this window
        ...
        # Now answer the CDF query for each k_int using the cached table
        ...
        for i, ki in enumerate(k_int):
            if ki < s_min:
                ...
                # ki is between s_min and s_max; index into cdf_support
    ...
    def _get_degrees(self, node_type: str) -> np.ndarray:
        ...
        mask = (self.node_types == type_val)
        return self.in_degrees[mask]
...
class GoFDiagnostics:
    ...
    def empirical_cdf_integers(self, data):
        """
        Compute the empirical CDF for integer data only (vectorized, less alloc).
        """
    ...
    def _fit_beta_mle(self,net: DirectedHomophilicNetwork,data,a: int,b: int,node_type: str,beta_init,):
        ...
        data = np.asarray(data, dtype=int)
        if data.size == 0:
            # If no data, just return network's theoretical parameters
            params0 = net._get_params(node_type)
            beta_mle = np.array(
                [params0['p0'], params0['alpha'], params0['gamma']],
                dtype=float,
            )
            return beta_mle, True

        # Theoretical starting params for this node type
        params0 = net._get_params(node_type)
        ...
        # Fixed integer support for this MLE
        k_support = np.arange(a, b + 1, dtype=int)

        # Sufficient statistics: counts per unique k in the data
        unique_k, counts = np.unique(data, return_counts=True)

        # Restrict to the window [a, b]
        mask_window = (unique_k >= a) & (unique_k <= b)
        unique_k = unique_k[mask_window]
        counts = counts[mask_window]

        # If, after truncation, there is still no data, just return initial beta
        if counts.size == 0:
            return np.array([p0_0, alpha0, gamma0], dtype=float), True

        # Precompute mapping from k -> index on the fixed support [a, b]
        k_to_idx = {k: i for i, k in enumerate(k_support)}

        # Precompute the index array once for all objective calls
        idxs = np.array([k_to_idx[k] for k in unique_k], dtype=int)

        # Precompute masks that depend only on k_support
        zero_mask = (k_support == 0)
        pos_mask = (k_support > 0)
        k_pos = k_support[pos_mask]

        def truncated_pmf(theta: np.ndarray) -> np.ndarray:
            """
            Vectorised PMF on [a, b], normalised over that window.
            Uses precomputed masks and k_support.
            """
            ...
            # k > 0 part
            if np.any(pos_mask):
                # betaln is vectorised over k_pos
                log_ratio = betaln_func(k_pos, alpha + gamma) - betaln_func(k_pos, alpha)
                pmf[pos_mask] = p0 * np.exp(log_ratio)

            Z_trunc = pmf.sum()
            if Z_trunc <= 0:
                # Degenerate case: return tiny uniform-ish probabilities
        ...
        def objective(theta: np.ndarray) -> float:
            """
            Negative log-likelihood of data under truncated PMF.
            Only depends on theta; k-support, idxs, etc. are precomputed.
            """
            model_pmf = truncated_pmf(theta)
            probs = model_pmf[idxs]
        ...
        return beta_mle.astype(float), used_fallback

    def _mc_single_sim(self,base_net_params: dict,node_type: str,a: int,b: int,beta_theory_arr: np.ndarray,seed: int = None,):
        """
        Run a single Monte Carlo simulation for window [a, b].
        Returns: (D_theory_s, D_mle_s, beta_mle_s, used_fallback_s)
        """

        # Create a fresh network with given parameters and optional seed
        net_sim = DirectedHomophilicNetwork(
            n0=base_net_params['n0'],
            n_nodes=base_net_params['n_nodes'],
            m_edges=base_net_params['m_edges'],
            h=base_net_params['h'],
            f_a=base_net_params['f_a'],
            mu_a=base_net_params['mu_a'],
            mu_b=base_net_params['mu_b'],
            seed=seed,
            build_graph=False,   # <--- key change
        )
        net_sim.generate_network()

        # Degrees of the chosen node type, truncated to [a, b]
        degrees = np.array(net_sim._get_degrees(node_type), dtype=int)
        data_s = degrees[(degrees >= a) & (degrees <= b)]

        if data_s.size == 0:
            # No data in window: degenerate case
            D_theory_s = 0.0
            D_mle_s = 0.0
            beta_mle_s = (
                beta_theory_arr
            ...
            used_fallback_s = True
            return D_theory_s, D_mle_s, beta_mle_s, used_fallback_s

        # CSN distance with theoretical beta
        D_theory_s = self.csn_distance(net_sim,data_s,a, b, beta_theory_arr, node_type,)

        # MLE fit on this simulated dataset
        beta_mle_s, used_fallback_s = self._fit_beta_mle( net_sim, data_s, a, b, node_type, beta_init=None,)

        # CSN distance with MLE beta
    ...
    def mc_on_window(self,net: DirectedHomophilicNetwork,node_type: str,a: int,b: int,
        beta_theory,N_sims: int,max_workers: int | None = None,):
        """
        Run Monte Carlo on window [a, b] with N_sims simulations.
        Parallelises over simulations using a process pool.
        """
        D_theory_list = []
        D_mle_list = []
        beta_mle_list = []
        ...
        # Ensure beta_theory is a NumPy array (or None)
        beta_theory_arr = (
            np.asarray(beta_theory, dtype=float)
        ...
        seeds = [rng.randint(0, 2**31 - 1) for _ in range(N_sims)]

        tasks = [
            (base_net_params, node_type, a, b, beta_theory_arr, seeds[s])
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(_mc_single_sim_worker, task)
                for task in tasks
            ]

            for fut in as_completed(futures):
                D_theory_s, D_mle_s, beta_mle_s, used_fallback_s = fut.result()
                results_per_sim.append(
                    (D_theory_s, D_mle_s, beta_mle_s, used_fallback_s)
                )

        # Aggregate results
        for D_theory_s, D_mle_s, beta_mle_s, used_fallback_s in results_per_sim:
        ...
        sigma_p = float(
            np.sqrt(p * (1.0 - p) / max(N_sims, 1))
        )

        # Stack beta estimates (avoid try/except in hot path)
        beta_mle_arr = np.asarray(beta_mle_list, dtype=float)
        if beta_mle_arr.ndim != 2:
            beta_mle_arr = np.array(beta_mle_list, dtype=object)

        frac_moved = (
            float(np.mean(~fallback_flags)) if fallback_flags.size > 0 else np.nan
        )
        ...
        if beta_theory_arr is not None and beta_mle_mean is not None:
            with np.errstate(divide='ignore', invalid='ignore'):
                percent_diff_mean_vs_theory = (
                    100.0 * ((beta_mle_mean - beta_theory_arr) / beta_theory_arr)
                )
        ...
        # Clear cache after MC simulations to prevent memory buildup
        net.clear_cache()
        return result
...
class NetworkStatistics:
    ...
    def print_statistics(self, net: DirectedHomophilicNetwork):
        ...
        for degrees, type_name in [(in_degrees_a, 'a'), (in_degrees_b, 'b')]:
            ...
            n_deg = degrees.size  # NumPy array

            print(
                f"\nType '{type_name}': {n_deg:,} nodes "
                f"({n_deg / n_total * 100:.1f}%)"
            )
            if n_deg == 0:
                mean_deg = 0.0
                max_deg = 0
            ...
            print(f"  Mean in-degree: {mean_deg:.2f}")
            print(f"  Max in-degree: {max_deg}")
            print(f"  Min in-degree: {min_deg}")
...
if __name__ == "__main__":
    config = dict(
        ...
        grid_2d=dict(
            ...
            node_type='b', a=0,
            p_c_list=[0.2, 0.4],
            N_sims=10, b_grid_type='linear', n_b=5,
